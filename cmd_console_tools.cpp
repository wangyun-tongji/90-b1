...
/***************************************************************************
  ...
  输入参数：const int X       ：X轴坐标（列）
		    const int Y       ：Y轴坐标（行）
		    const char *src   ：要输出的字符串
		    const int bg_color：背景色（缺省为COLOR_BLACK）
		    const int fg_color：背景色（缺省为COLOR_WHITE）
			const int rpt     ：重复次数（缺省为1）
			const int max_len ：-1（表示不限制长度，按srclen(src)*rpt的实际打印）
  返 回 值：
  说    明：如果读不懂，暂时放着，等指针学习完成后再阅读
***************************************************************************/
void cct_showsrc(const int X, const int Y, const char *src, const int bg_color, const int fg_color, int rpt, int maxlen)
{
	const char *p;
	int i, rpt_count = 0;
	...
	/* 之行到此，是src非NULL/src!=""的情况(既srclen一定>0) */
	if (rpt <= 0)
		rpt = 1; //防止错误参数

	if (maxlen < 0)
		maxlen = strlen(src) * rpt; //未给出maxlen则为原始长度

	for (i = 0, p = src; i < maxlen; i++, p++) {	//重复rpt次，每次输出字符串，适用于在画边框时输出若干个"═"等情况
		if (*p == 0) {
			p = src; //如果p已经到\0，则回到头（此处已保证srclen(src)>0，即一定有内容）
			rpt_count++;
			}
...
/***************************************************************************
  ...
  返 回 值：
  说    明：下列说明来自鼠标定义文件
			01.typedef srcuct _MOUSE_EVENT_RECORD      //鼠标事件结构体
			02.{
...
int cct_read_keyboard_and_mouse(int &MX, int &MY, int &MAction, int &keycode1, int &keycode2)
{
	...
	while (1) {
		...
		if (InputRec.EventType == MOUSE_EVENT) {
			...
			/* https://docs.microsoft.com/en-us/windows/console/mouse-event-record-src
